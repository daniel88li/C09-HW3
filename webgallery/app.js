...
app.post("/signup/", function (req, res, next) {
  ...
  users.findOne({ _id: username }, function (err, user) {
    ...
    users.update(
      { _id: username },
      { _id: username, hash: hash, salt: salt },
      { upsert: true },
      function (err) {
        if (err) return res.status(500).end(err);
  ...
  images
    ...
    .exec(function (err, image) {
      ...
      if (image.length == 0)
        return res
          .status(404)
          .end("Image page " + req.query.page + " or username does not exist");
      return res.json(image);
    });
});

app.get(
  "/api/:username/firstImage/",
  isAuthenticated,
  function (req, res, next) {
    images
      .find({ author: req.params.username })
  ...
  images.find({ _id: req.params.imageId }, function (err, image) {
    ...
    if (image.length == 0) {
      ...
      comments
        ...
        .sort({ createdAt: -1 })
        .skip(10 * parseInt(req.query.page))
        .limit(10)
  ...
  images.findOne({ _id: req.params.id }, function (err, image) {
    ...
    if (image.author != req.username) {
      ...
      images.remove({ _id: image._id }, function (err, num) {
        images
          .findOne({ createdAt: { $gte: oldDate } })
          .sort({ createdAt: -1 })
          .exec(function (err, nextImage) {
            if (!nextImage) {
              images
                .findOne({ createdAt: { $lte: oldDate } })
                .sort({ createdAt: -1 })
                .exec(function (err, prevImage) {
...
app.delete("/api/comments/:id/", isAuthenticated, function (req, res, next) {
  comments.findOne({ _id: req.params.id }, function (err, comment) {
    ...
    images.findOne({ _id: comment.imageId }, function (err, image) {
      if (err) return res.status(500).end(err);
      if (!image)
        return res.status(404).end("Image id for comment does not exist");
